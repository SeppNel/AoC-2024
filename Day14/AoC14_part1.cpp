...
int main() {
    ...
    for (const Robot &r : input) {
        pair<int64_t, int64_t> pos = solve(r, 100);

        if (inVector(pos.first, centerX) || inVector(pos.second, centerY)) {
            continue;
        }

        if (pos.first < min(centerX) && pos.second < min(centerY)) {
            q1++;
        } else if (pos.first < min(centerX) && pos.second > max(centerY)) {
            q2++;
        } else if (pos.first > max(centerX) && pos.second < min(centerY)) {
            q3++;
        } else if (pos.first > max(centerX) && pos.second > max(centerY)) {
            q4++;
        }
    }
